...
// Movies genres
// https://api.themoviedb.org/3/genre/movie/list?api_key=19f84e11932abbc79e6d83f82d6d1045&language=en-US
function showMovieNames(movies) {
...
document.getElementById('searchInput').addEventListener('input', () => {
  ...
  if (searchQuery !== '') {
      const url = `https://api.themoviedb.org/3/search/movie?api_key=19f84e11932abbc79e6d83f82d6d1045&language=en-US&page=1&query=${searchQuery}`;
      ...
      fetch(url)
          ...
          .then(data => {
              showMovieNames(data.results);
              searchResults.classList.add('show'); // Show the search results
          })
          .catch(error_data => {
              console.log(error_data);
          });
...
document.body.addEventListener('click', (event) => {
  ...
  // Check if the clicked element is not within the search bar or its results
  if (!searchContainer.contains(event.target) &&
      event.target !== searchInput &&
      event.target !== searchButton &&
      event.target !== searchResults) {
      // Close the search bar by clearing the search input and results
      searchInput.value = '';
      searchResults.innerHTML = '';
  }
});
